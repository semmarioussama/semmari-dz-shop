...
const ProductForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/thank-you?ref=${data.orderReference}&name=${encodeURIComponent(validation.data.fullName)}&phone=${encodeURIComponent(validation.data.phone)}&value=${5700 * quantity}`);
    } catch (error) {
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-2 border border-border">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">السعر ({quantity} × 5,700 دج)</span>
              <span className="font-medium text-foreground">{(quantity * 5700).toLocaleString('ar-DZ')} دج</span>
            </div>
            ...
            <div className="border-t border-border pt-2 mt-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-lg text-red-600">
                  {(() => {
                    const itemsTotal = quantity * 5700;
